<template>
  <div class="cfg-page">
    <!-- ── 顶部标题栏 ── -->
    <PageHeader title="欢迎消息 (MOTD)" subtitle="motd.txt">
      <template #meta>
    ...
    <!-- Agent 离线 -->
    <AgentOfflineNotice v-if="!props.agentOnline" message="Agent 未连接，无法读取或保存 MOTD。请先启动服务器。" />
    ...
    <template v-else>
      ...
      <!-- 读取失败 -->
      <AgentOfflineNotice v-else-if="loadError" type="error" :message="loadError" show-retry :retry-label="'重试'" @retry="loadMotd" />
