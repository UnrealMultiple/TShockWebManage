...
namespace TerrariaManagerAgent.Services
{
    ...
    public class RuntimeBroadcastService
    {
        ...
        private const int StatusBroadcastIntervalMs = 15000;
        ...
        public void OnPlayerPostLogin(PlayerPostLoginEventArgs args)
        {
            ...
            if (!string.IsNullOrWhiteSpace(args.Player.Name))
                StatsTracker.OnJoin(args.Player.Name);
            _ = BroadcastStatusOnce();
            _ = BroadcastStatusAfterDelay(800);
        }

        public void OnPlayerLogout(PlayerLogoutEventArgs args)
        {
            if (args?.Player == null) return;
            _ = BroadcastStatusOnce();
        }

        private async Task BroadcastStatusAfterDelay(int delayMs)
        {
            try
            {
                await Task.Delay(delayMs);
                await BroadcastStatusOnce();
            }
            catch
            {
            }
        ...
        public async Task StatusBroadcastLoop(CancellationToken token)
        {
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                try { await Task.Delay(StatusBroadcastIntervalMs, token); }
                catch (OperationCanceledException) { break; }
            }
        }

        public async Task BroadcastStatusOnce()
        {
            try
            {
                _thisProcess ??= Process.GetCurrentProcess();
